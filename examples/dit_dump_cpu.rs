...
use maolan_generate::acestep::dit::{AceStepDiT, TURBO_TIMESTEPS};
...
fn main() -> anyhow::Result<()> {
    ...
    let dit =
        AceStepDiT::<B>::from_burnpack(&dit_config, &model_dir.join("acestep-dit.bpk"), &device)?;
    ...
    // ---- DiT loop with per-step dumps ----
    let kv = dit.prepare_cross_kv(enc);
    let mut xt = noise;
    let total = TURBO_TIMESTEPS.len();
    for (index, &t_cur) in TURBO_TIMESTEPS.iter().enumerate() {
